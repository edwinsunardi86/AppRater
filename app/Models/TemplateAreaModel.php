...
class TemplateAreaModel extends Model
{
    ...
    static function getDataTemplateArea2(){
        ...
        ->groupBy('location_id','a.id')->get();
        return $query;
    }

    static function getDataDetailTemplateArea($id){
        $query = DB::table('header_template AS a')
        ->join('setup_location AS b','a.location_id','=','b.id')
    ...
    static function getDataServiceByTemplate($location_id,$id_header_template){
        ...
        if($id_header_template){
            $query = $query->where('b.id',$id_header_template);
        }else{
            $query = $query->where('c.id',$location_id);
        }
        $query = $query->groupBy('service_code')->get();
        return $query;
    }
    
    
    static function getLocationByTemplate($project_code){
        $query = DB::table('header_template AS a')
        ...
        ->select(DB::Raw('a.id AS id_header_template'),'region_name','a.location_id','c.location_name','start_date','finish_date',DB::Raw("GROUP_CONCAT(b.area_name) AS area"),'start_date','finish_date')
